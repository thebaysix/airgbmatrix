...
#
# Hook runners may start this script before notify.sh has finished POSTing the
# new session. Retry the lookup briefly to cover that race; each request is
# capped at 500ms so the whole loop remains below the 5s hook timeout.
set -uo pipefail
...
for attempt in 1 2 3 4 5; do
  RESP=$(curl -sS --max-time 0.5 "http://${HOST}:${PORT}/sessions" 2>/dev/null || true)
  CI=$(printf '%s' "$RESP" \
